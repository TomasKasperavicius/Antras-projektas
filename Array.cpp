...
struct Studentas
{
    ...
    double *pazymiai, egzaminas, galutinis;
};

void pazymiu_generavimas(Studentas *, int, int);
void Isvedimas(Studentas *, int, std::string, std::string, int);
void AtlaisvintiAtminti(Studentas *, int);
void MasyvoDidinimas(Studentas *, int &);
...
int main()
{
        ...
        else if (nd_pazymiai == "Ne")
        {
            if (i == 0)
            {
                ...
                while (true)
                {
                    ...
                    generavimas == "Taip" ? cout << "Ar generuoti " << n + 1 << " namu darbu pazymi? [Taip/Ne]: " : cout << "Ar ivesite " << n + 1 << " namu darbu pazymi? [Taip/Ne]: ";
                    cin >> vedimas;
                    if (vedimas == "Ne")
                    {
                        break;
                    ...
                    if (generavimas == "Taip")
                    {
                        MasyvoDidinimas(st, n);
                        st[i].pazymiai[n - 1] = 1 + rand() % 10;
                    }
                    else
                    {
                        MasyvoDidinimas(st, n);
                        cout << n << " namu darbu pazymys: ";
                        cin >> st[i].pazymiai[n - 1];
                        while (!cin || st[i].pazymiai[n - 1] < 1 || st[i].pazymiai[n - 1] > 10)
                        {
                            cin.clear();
                            cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                            cout << "Namu darbo pazymys gali buti tik skaicius desimtbaleje sistemoje! ";
    ...
    AtlaisvintiAtminti(st, studentu_kiekis);
    return 0;
}
void pazymiu_generavimas(Studentas *st, int kelintas, int n)
{
    for (int i = 0; i < n; i++)
    {
        st[kelintas].pazymiai[i] = 1 + rand() % 10;
    }
}
void AtlaisvintiAtminti(Studentas *st, int n)
{
    for (int i = 0; i < n; i++)
    {
        delete[] st[i].pazymiai;
    }
    delete[] st;
}
void MasyvoDidinimas(Studentas *st, int &n)
{
    double *temp = new double[n];
    for (int i = 0; i < n; i++)
    {
        temp[i] = st[0].pazymiai[i];
    }
    delete[] st[0].pazymiai;
    st[0].pazymiai = new double[++n];
    for (int i = 0; i < n; i++)
    {
        st[0].pazymiai[i] = temp[i];
    }
    delete[] temp;
}
void Isvedimas(Studentas *st, int studentu_kiekis, std::string vid_med, std::string generavimas, int n)
{
    ...
    else if (vid_med == "med")
    {
        ...
        for (int i = 0; i < studentu_kiekis; i++)
        {
            if (n != 0)
            {
                std::sort(st[i].pazymiai, st[i].pazymiai + n);
                if (n % 2 == 0)
                {
